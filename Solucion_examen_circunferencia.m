...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TIEMPO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tf = 16;             % Tiempo de simulacion (ajustado al dominio)
ts = 0.01;
t = 0:ts:tf;
...
for k = 1:N
    ...
    %y=±((16 - t^2)^1/2);
    %t= [-4 a 4]
    %θ(𝒕) = t𝒂𝒏^−𝟏(t / sqrt(16 - t^2))
  
    tk = t(k);

   % -------- PARTE SUPERIOR --------
    if tk <= 7.999
    tNorm = -3.999 + tk;        % tiempo normalizado arriba [-4 a 4]

    dx = 1;
    dy = -(tNorm) / sqrt(16 - (tNorm)^2);
    
    % Velocidad lineal
    u(k) = sqrt(dx^2 + dy^2);
    
    % Velocidad angular
    w(k) = (-16 / (16 - (tNorm)^2)^(3/2)) / (1 + ((tNorm) / sqrt(16 - (tNorm)^2))^2);

        
    % -------- PARTE INFERIOR --------
    else
        tNorm = 3.999 - (tk - 7.999);   % tiempo normalizado abajo [4 a -4]

        dx = 1;
        dy = -tNorm/ sqrt(16 - (tNorm)^2);   % CAMBIA SIGNO
...
axis([-5 5 -5 5 0 4]); % Ingresar limites minimos y maximos en los ejes x y z [minX maxX minY maxY minZ maxZ]
